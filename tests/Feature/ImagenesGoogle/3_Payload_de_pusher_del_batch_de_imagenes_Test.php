...
class Payload_de_pusher_del_batch_de_imagenes_Test extends TestCase
{
     ...
     * nombres y resúmenes largos a propósito (mismo orden de magnitud que un artículo real,
     * ver los logs del 25/8/2026 con nombres de 80+ caracteres).
     *
    ...
    public function el_tamano_del_payload_no_depende_de_la_cantidad_de_articulos()
    {
        ...
        // No se compara con igualdad exacta: los contadores son números y "5" ocupa menos
        // dígitos que "400" en JSON. La diferencia tolerada (20 bytes) cubre esa variación de
        // dígitos varias veces de sobra; lo que este assert descarta es que el payload vuelva a
        // crecer con la cantidad de artículos, que es la causa real del bug (cada artículo de
        // más agrega decenas o cientos de bytes, no unos pocos).
        $this->assertLessThan(20, abs($tamano_grande - $tamano_chico));
    }
    ...
    public function el_payload_lleva_los_contadores_y_el_batch_uuid()
    {
        ...
        $this->assertSame(3, $payload['needs_review']);
        $this->assertSame(true, $payload['quota_reached']);
        $this->assertSame(3, $payload['skipped_by_quota']);
