...
class SaleTicketPdf extends fpdf {
	...
	function logo() {
		...
		if (!is_null($this->user->image_url)) {
			if (config('app.APP_ENV') == 'local') {
				...
				$this->Image($this->user->image_url, $logo_x, $this->x_incial, $logo_w, $logo_h);
			}
	 ...
	 * Usa getimagesize() sobre la imagen del usuario; si falla (URL no accesible, formato
	 * invalido, etc.) cae a un fallback cuadrado para no romper ni el render ni el calculo
	 * de altura del PDF (getPdfHeight).
