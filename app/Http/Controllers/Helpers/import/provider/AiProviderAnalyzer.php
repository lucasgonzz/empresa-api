...
namespace App\Http\Controllers\Helpers\import\provider;
 ...
 *
 * No gestiona lógica de artículos ni de clientes.
...
class AiProviderAnalyzer
{
    ...
    /**
     * Lista de propiedades del sistema importables para proveedores que Claude puede identificar.
    ...
    protected const SYSTEM_PROPERTIES = [
        ...
        /*
         * Saldo de cuenta corriente del proveedor: lo que se le debe al arrancar.
         *
         * Lo consume ProviderImport vía LocalImportHelper::setSaldoInicial(), que lo busca
         * con los alias ['saldo_actual', 'saldo actual']. Esta clave TIENE que ser una de
         * esas dos: con cualquier otra, la columna se mapea, el usuario la ve mapeada, y el
         * saldo no se importa — en silencio, sin ningún error.
         *
         * ⚠️ Va sólo acá y en AiClientAnalyzer. NO en AiExcelAnalyzer: un artículo no tiene
         * cuenta corriente. Es el caso inverso al de los mensajes de error de esta misma
         * misión, donde cada cambio va en los tres clones.
         */
        'saldo_actual',
        'observaciones',
    ];
    ...
    public function analyze(string $excel_path, string $original_filename = '', array $opciones = []): array
    {
         ...
         *
         * 🔴 ESTO NO ES DE ARTÍCULOS SOLAMENTE. La propagación de fusiones se había
         * entregado sólo en AiExcelAnalyzer, y este analizador le mandaba a Claude el nombre
         * vacío de la columna cubierta por la fusión ([..., "PRECIOS", ""]) y no devolvía
         * ni un aviso: al usuario de proveedores se le mostraba la misma pantalla que al de
         * artículos, prometiéndole lo mismo y sin cumplirlo. Lo que vale para artículos
         * vale para los tres modelos.
         */
         ...
         * compartido: si acá faltan, el usuario de proveedores ve el selector de hoja y la
         * alerta amarilla vacíos sobre un archivo que sí tiene el problema, y falla EN
         * SILENCIO: no tira ningún error, sólo no avisa. Se agregan; ninguna clave
         * existente cambia.
         */
        $parsed['hojas'] = $hojas;
     ...
     * catálogo sin un error en pantalla; acá son dos datos del proveedor cruzados, que se
     ...
     *
     * No incluye lógica de inferencia de proveedores; se centra en propiedades de proveedores.
     *
    ...
    protected function build_prompt(array $sample_data, string $original_filename = ''): string
    {
         ...
         */
        $prompt = <<<PROMPT
Analizá el siguiente archivo Excel de importación de proveedores y devolvé SOLO un JSON válido (sin markdown, sin explicaciones extra).
...
3. La propiedad más importante es "nombre" — es el nombre comercial o razón social del proveedor.
4. La propiedad "numero" corresponde al número o código de proveedor (identificador interno).
5. Para "condicion_frente_al_iva": mapeá columnas como "Condición IVA", "IVA", "Tipo IVA" a esta propiedad.
6. Para "observaciones": mapeá columnas de notas, comentarios o información adicional del proveedor.
7. Para "localidad": mapeá columnas de ciudad, localidad o ubicación del proveedor.
8. Para "saldo_actual": mapeá columnas de saldo, deuda o cuenta corriente a esta propiedad.
...
- provider_id debe ser siempre null (no aplica para importación de proveedores)
- provider_confidence debe ser siempre "bajo" (no aplica para importación de proveedores)
...
PROMPT;
     ...
     *
     * Para proveedores, provider_id es siempre null y provider_confidence siempre 'bajo'.
     *
    ...
    protected function parse_claude_response(string $claude_text): array
    {
        ...
        if (!isset($parsed['column_mapping']) || !is_array($parsed['column_mapping'])) {
            Log::error('AiProviderAnalyzer: la respuesta de Claude no trae column_mapping', [
                'raw_response' => $claude_text,
            ]);
        ...
        /*
         * Para importación de proveedores no hay proveedor inferido.
     ...
     *
     * No aplica normalización de alias ya que las propiedades de proveedores no requieren alias especiales.
     *
    ...
    protected function enrich_column_mapping(array $column_mapping, array $headers): array
    {
         ...
         * índice de cada nombre y descartaba los demás. Con una cabecera fusionada sobre
         * dos columnas, la propagación deja el mismo nombre en las dos, y los dos ítems que
         * Claude devuelva con ese nombre se llevaban el mismo índice: dos propiedades del
         * sistema leyendo la MISMA celda del Excel, sin un error en pantalla. Por eso el mapa
         * es una lista y se consume en orden. De paso arregla el bug latente de cualquier
         * planilla que repita un nombre de columna, sin fusiones de por medio.
         *
        ...
        foreach ($column_mapping as $array_position => $mapping_item) {
            ...
            if ($normalized_excel_name !== '' && isset($header_indexes_by_name[$normalized_excel_name])) {
                ...
                if ($ya_repartidos < count($indices_del_nombre)) {
                    ...
                    /*
                     * Claude devolvió más ítems con ese nombre que columnas con ese
                     * nombre hay en el archivo (se lo inventó). No hay índice libre que
                     * darle: se repite el último, que es el comportamiento de siempre para un
                     * ítem de más, y detectar_columnas_ambiguas() lo denuncia en el aviso
                     * del paso 2.
                     */
            ...
            if (!is_null($system_property)) {
                ...
                /* Descartamos propiedades que no están en el contrato del importador de proveedores. */
