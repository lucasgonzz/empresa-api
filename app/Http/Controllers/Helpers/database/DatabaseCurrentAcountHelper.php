...
class DatabaseCurrentAcountHelper {
    ...
    static function copiar_current_acounts($user, $bbdd_destino, $from_id) {
        $current_acounts = CurrentAcount::where('user_id', $user->id)
                        ->orderBy('id', 'ASC')
                        ->where('id', '>=', $from_id)
        ...
        foreach ($current_acounts as $current_acount) {
            ...
            // foreach ($current_acount->pagado_por as $pagado_por) {

            //     $current_acount->pagado_por()->attach($this->pago->id, [
            //         'pagado'        => $pagado,
            //         'total_pago'    => $this->pago->haber,
            //         'created_at'    => $this->pago->created_at,
            //     ]);

            //     $created_pagado_por = PagadoPor::create($pagado_por->toArray());
            //     echo 'Se creo pagado_por id '.$created_pagado_por->id.' </br>';
            // }

            // foreach ($current_acount->pagando_a as $pagando_a) {
            //     $created_pagado_por = PagadoPor::create($pagando_a->toArray());
            //     echo 'Se creo pagando_a id '.$created_pagado_por->id.' </br>';
            // }
